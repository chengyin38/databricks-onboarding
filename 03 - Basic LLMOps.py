...
prod_data_path = f"/Users/{user_name}/LLMOps/prod_data"
...
mlflow.set_experiment(f"/Users/{user_name}/LLMOps MLflow experiment")
...
# MAGIC %md
# MAGIC ## Inspect summary with MLflow

# COMMAND ----------

xsum_test_sample = xsum_dataset["test"].select(range(10))

# Evaluate a language model
mlflow.evaluate(model_info.model_uri, data=xsum_test_sample.to_pandas(), model_type="text")

# COMMAND ----------

# Download evaluation results for further analysis
display(mlflow.load_table("eval_results_table.json"))
...
model_name = f"summarizer - {user_name}"
...
inference_results_path = f"/Users/{user_name}/LLMOps/inference-results".replace(
    "/dbfs", "dbfs:"
)
